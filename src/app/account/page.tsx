...
export default async function AccountPage() {
  ...
  const [profileSnapshot, historySnapshot] = await Promise.all([
    playerRef.get(),
    playerRef.collection("mockTopUps").orderBy("createdAt", "desc").limit(5).get(),
  ]);
  ...
  const mockGold = typeof profile?.mockGold === "number" ? profile.mockGold : 0;
  ...
  const history = historySnapshot.docs.map((document) => {
    ...
    return {
      id: document.id,
      packageName: typeof data.packageName === "string" ? data.packageName : "Mock package",
      ...
      <div className="mx-auto max-w-4xl px-5 sm:px-8">
        <section className="game-panel mt-16 overflow-hidden rounded-3xl">
          ...
          <div className="grid gap-px bg-white/7 sm:grid-cols-4">
            ...
            <div className="bg-[#0a1711] p-6"><p className="text-[10px] font-bold uppercase tracking-wider text-stone-600">Mock gold</p><p className="mt-2 text-xl font-black text-amber-300">◆ {mockGold.toLocaleString()}</p></div>
        ...
        <section className="game-panel mt-5 overflow-hidden rounded-2xl"><div className="border-b border-white/7 px-5 py-4"><h2 className="font-bold text-white">Recent mock top-ups</h2><p className="mt-1 text-xs text-stone-600">Development transactions only—no real payments</p></div>{history.length ? <div className="divide-y divide-white/6">{history.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 px-5 py-4"><div><p className="text-sm font-bold text-stone-300">{item.packageName}</p><p className="mt-1 text-xs text-stone-600">Mock · completed</p></div><span className="font-mono text-sm font-bold text-amber-300">+{item.gold.toLocaleString()} gold</span></div>)}</div> : <p className="px-5 py-8 text-center text-sm text-stone-600">No mock top-ups yet.</p>}</section>
